...
public:
    ...
    static Tensor fromArray(std::initializer_list<ComponentType> values)
    {
        std::vector<ComponentType> data(values);
        Tensor t({data.size()});
    ...
    static Tensor fromArray(std::initializer_list<std::initializer_list<ComponentType>> values)
    {
        ...
        {
            assert(row.size() == cols && "All rows must have the same number of columns.");
        }
...
template <Arithmetic ComponentType>
size_t
Tensor<ComponentType>::rank() const { return shape_.size(); }

template <Arithmetic ComponentType>
std::vector<size_t>
Tensor<ComponentType>::shape() const { return shape_; }

template <Arithmetic ComponentType>
size_t
Tensor<ComponentType>::numElements() const { return numTensorElements(shape_); }
